...
void InstancingApp::Render()
{
  ...
  VkBuffer vertexStreams[] = {
    m_teapot.vertexBuffer.buffer, m_instanceData.buffer
  };
  vkCmdBindVertexBuffers(command, 0, 
    _countof(vertexStreams), vertexStreams, offsets);
...
void InstancingApp::PrepareInstanceData()
{
  ...
  auto stageBuf = CreateBuffer(bufferSize, usageVB | VK_BUFFER_USAGE_TRANSFER_SRC_BIT, srcMemoryProps);
...
{
  auto vertexStride = uint32_t(sizeof(TeapotModel::Vertex));
  auto instanceStride = uint32_t(sizeof(InstanceData));
  std::array<VkVertexInputBindingDescription, 2> vibDesc{
    {
      { 0, vertexStride, VK_VERTEX_INPUT_RATE_VERTEX },
      { 1, instanceStride, VK_VERTEX_INPUT_RATE_INSTANCE }
    }
  };

  array<VkVertexInputAttributeDescription, 4> inputAttribs{
    {
      ...
      { 2, 1, VK_FORMAT_R32G32B32_SFLOAT, offsetof(InstanceData, offsetPosition) },
      { 3, 1, VK_FORMAT_R32G32B32A32_SFLOAT, offsetof(InstanceData, color) },
    }
  };
  VkPipelineVertexInputStateCreateInfo pipelineVisCI{
    VK_STRUCTURE_TYPE_PIPELINE_VERTEX_INPUT_STATE_CREATE_INFO,
    nullptr, 0,
    uint32_t(vibDesc.size()), vibDesc.data(),
